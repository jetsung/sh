...
#============================================================
# File: yq.sh
# Description: 便携式命令行 YAML/JSON/XML/CSV/TOML 等多种格式处理工具
...
download_exact() {
    ...
    TMP_DIR=$(mktemp -d /tmp/yq.XXXXXX)
...
main() {
    ...
    if [[ -z "$DOWNLOAD_URL" ]]; then
        ...
        DOWNLOAD_URL="$(get_download_url mikefarah/yq "$OS_RE" "$ARCH_RE")"

        if [[ -z "$DOWNLOAD_URL" || "$DOWNLOAD_URL" == "null" ]]; then
            echo "Error: 未在 mikefarah/yq 的最新 release 中找到匹配 $OS-$ARCH 的安装包"
            exit 1
        fi
    else
    ...
    if ! check_is_command "yq"; then
        echo "yq has not been installed successfully."
        echo ""
        exit 1
    fi
