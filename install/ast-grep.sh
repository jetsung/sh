...
#============================================================
# File: ast-grep.sh
# Description: 基于语法树的代码结构搜索、lint 与重写 CLI 工具（ast-grep）
...
download_exact() {
    local file_bin="ast-grep"
    local download_file="package.zip"
    TMP_DIR=$(mktemp -d /tmp/ast-grep.XXXXXX)
...
main() {
    ...
    if [[ -z "$DOWNLOAD_URL" ]]; then
        ...
        DOWNLOAD_URL="$(get_download_url ast-grep/ast-grep "$OS_RE" "$ARCH_RE")"

        if [[ -z "$DOWNLOAD_URL" || "$DOWNLOAD_URL" == "null" ]]; then
            echo "Error: 未在 ast-grep/ast-grep 的最新 release 中找到匹配 $OS-$ARCH 的安装包"
            exit 1
        fi
    else
    ...
    if ! check_is_command "ast-grep"; then
        echo "ast-grep has not been installed successfully."
        echo ""
        exit 1
    fi

    echo "ast-grep has been installed successfully!"
    echo ""
    ast-grep --help || true
    echo ""
    ast-grep --version || true
    echo ""
}
