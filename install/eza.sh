...
#============================================================
# File: eza.sh
# Description: ls 的现代替代工具
# Source: https://github.com/eza-community/eza
# URL: https://fx4.cn/eza
...
download_exact() {
    ...
    TMP_DIR=$(mktemp -d /tmp/eza.XXXXXX)
...
main() {
    ...
    if [[ -z "$DOWNLOAD_URL" ]]; then
        ...
        DOWNLOAD_URL="$(get_download_url eza-community/eza "$OS_RE" "$ARCH_RE")"

        if [[ -z "$DOWNLOAD_URL" || "$DOWNLOAD_URL" == "null" ]]; then
            echo "Error: 未在 eza-community/eza 的最新 release 中找到匹配 $OS-$ARCH 的安装包"
            exit 1
        fi
    else
    ...
    if ! check_is_command "eza"; then
        echo "eza has not been installed successfully."
        echo ""
        exit 1
    fi

    echo "eza has been installed successfully!"
    echo ""
    eza --help || true
    echo ""
    eza --version || true
