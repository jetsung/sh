...
#============================================================
# File: procs.sh
# Description: 用 Rust 编写的现代 ps 进程查看替代工具
# Source: https://github.com/dalance/procs
# URL: https://fx4.cn/procs
...
download_exact() {
    local file_bin="procs"
    local download_file="package.zip"
    TMP_DIR=$(mktemp -d /tmp/procs.XXXXXX)
...
main() {
    ...
    if [[ -z "$DOWNLOAD_URL" ]]; then
        ...
        DOWNLOAD_URL="$(get_download_url dalance/procs "$OS_RE" "$ARCH_RE")"

        if [[ -z "$DOWNLOAD_URL" || "$DOWNLOAD_URL" == "null" ]]; then
            echo "Error: 未在 dalance/procs 的最新 release 中找到匹配 $OS-$ARCH 的安装包"
            exit 1
        fi
    else
    ...
    if ! check_is_command "procs"; then
        echo "procs has not been installed successfully."
        echo ""
        exit 1
    fi
