...
#============================================================
# File: lsd.sh
# Description: 下一代 ls 命令：彩色、带图标、更现代的目录列表工具
# Source: https://github.com/lsd-rs/lsd
# URL: https://fx4.cn/lsd
...
download_exact() {
    ...
    TMP_DIR=$(mktemp -d /tmp/lsd.XXXXXX)
...
main() {
    ...
    if [[ -z "$DOWNLOAD_URL" ]]; then
        ...
        DOWNLOAD_URL="$(get_download_url lsd-rs/lsd "$OS_RE" "$ARCH_RE")"

        if [[ -z "$DOWNLOAD_URL" || "$DOWNLOAD_URL" == "null" ]]; then
            echo "Error: 未在 lsd-rs/lsd 的最新 release 中找到匹配 $OS-$ARCH 的安装包"
            exit 1
        fi
    else
    ...
    if ! check_is_command "lsd"; then
        echo "lsd has not been installed successfully."
        echo ""
        exit 1
    fi
