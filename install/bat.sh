...
#============================================================
# File: bat.sh
# Description: 带语法高亮的 cat 增强工具
# Source: https://github.com/sharkdp/bat
# URL: https://fx4.cn/bat
...
download_exact() {
    ...
    TMP_DIR=$(mktemp -d /tmp/bat.XXXXXX)
...
main() {
    ...
    if [[ -z "$DOWNLOAD_URL" ]]; then
        ...
        DOWNLOAD_URL="$(get_download_url sharkdp/bat "$OS_RE" "$ARCH_RE")"

        if [[ -z "$DOWNLOAD_URL" || "$DOWNLOAD_URL" == "null" ]]; then
            echo "Error: 未在 sharkdp/bat 的最新 release 中找到匹配 $OS-$ARCH 的安装包"
            exit 1
        fi
    else
    ...
    if ! check_is_command "bat"; then
        echo "bat has not been installed successfully."
        echo ""
        exit 1
    fi
