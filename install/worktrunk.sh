...
#============================================================
# File: worktrunk.sh
# Description: 管理 Git worktree 的 CLI，专为并行 AI Agent 工作流设计
...
download_exact() {
    ...
    TMP_DIR=$(mktemp -d /tmp/wt.XXXXXX)
    ...
    # 附带安装包内其它可执行文件（如 git-wt），未找到时静默跳过
    while IFS= read -r _extra; do
        _name="$(basename "$_extra")"
        if [[ "$_name" == "$file_bin" ]]; then
            continue
        fi
        echo "安装附加二进制: ${_name}"
        sudo_exec install -m 0755 "$_extra" "/usr/local/bin/${_name}"
...
main() {
    ...
    if [[ -z "$DOWNLOAD_URL" ]]; then
        ...
        DOWNLOAD_URL="$(get_download_url max-sixty/worktrunk "$OS_RE" "$ARCH_RE")"

        if [[ -z "$DOWNLOAD_URL" || "$DOWNLOAD_URL" == "null" ]]; then
            echo "Error: 未在 max-sixty/worktrunk 的最新 release 中找到匹配 $OS-$ARCH 的安装包"
            exit 1
        fi
    else
    ...
    if ! check_is_command "wt"; then
        echo "wt has not been installed successfully."
        echo ""
        exit 1
    fi
