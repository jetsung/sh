...
#============================================================
# File: bottom.sh
# Description: 跨平台的图形化进程/系统监控工具
# Source: https://github.com/ClementTsang/bottom
# URL: https://fx4.cn/bottom
...
download_exact() {
    local file_bin="bottom"
    local download_file="package.tar.gz"
    TMP_DIR=$(mktemp -d /tmp/bottom.XXXXXX)
...
main() {
    ...
    if [[ -z "$DOWNLOAD_URL" ]]; then
        ...
        DOWNLOAD_URL="$(get_download_url ClementTsang/bottom "$OS_RE" "$ARCH_RE")"

        if [[ -z "$DOWNLOAD_URL" || "$DOWNLOAD_URL" == "null" ]]; then
            echo "Error: 未在 ClementTsang/bottom 的最新 release 中找到匹配 $OS-$ARCH 的安装包"
            exit 1
        fi
    else
    ...
    if ! check_is_command "bottom"; then
        echo "bottom has not been installed successfully."
        echo ""
        exit 1
    fi

    echo "bottom has been installed successfully!"
    echo ""
    bottom --help || true
    echo ""
    bottom --version || true
