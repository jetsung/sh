...
#============================================================
# File: gix.sh
# Description: 纯 Rust 实现的轻量快速 Git 工具集
...
main() {
    ...
    if [[ -z "$DOWNLOAD_URL" ]]; then
        ...
        DOWNLOAD_URL="$(get_download_url GitoxideLabs/gitoxide "$OS_RE" "$ARCH_RE")"

        if [[ -z "$DOWNLOAD_URL" || "$DOWNLOAD_URL" == "null" ]]; then
            echo "Error: 未在 GitoxideLabs/gitoxide 的最新 release 中找到匹配 $OS-$ARCH 的安装包"
            exit 1
        fi
    else
    ...
    if ! check_is_command "gix"; then
        echo "gix has not been installed successfully."
        echo ""
        exit 1
    fi

    echo "gix has been installed successfully!"
    echo ""
    gix --help || true
    echo ""
    gix --version || true
