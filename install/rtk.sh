...
#============================================================
# File: rtk.sh
# Description: 降低 LLM token 消耗的 CLI 代理工具
# Source: https://github.com/rtk-ai/rtk
# URL: https://fx4.cn/rtk
...
get_download_url() {
    ...
    curl -fsSL "$repo_api_url" | jq -r --arg os "$os_re" --arg arch "$arch_re" '
        .assets[]?
        | select(.name | test("(?i)\\.(sha256|sha512|asc|sig|pem|json|txt|yaml|yml)$") | not)
...
download_exact() {
    ...
    TMP_DIR=$(mktemp -d /tmp/rtk.XXXXXX)
...
main() {
    ...
    if [[ -z "$DOWNLOAD_URL" ]]; then
        ...
        DOWNLOAD_URL="$(get_download_url rtk-ai/rtk "$OS_RE" "$ARCH_RE")"

        if [[ -z "$DOWNLOAD_URL" || "$DOWNLOAD_URL" == "null" ]]; then
            echo "Error: 未在 rtk-ai/rtk 的最新 release 中找到匹配 $OS-$ARCH 的安装包"
            exit 1
        fi
    else
    ...
    if ! check_is_command "rtk"; then
        echo "rtk has not been installed successfully."
        echo ""
        exit 1
    fi

    echo "rtk has been installed successfully!"
    echo ""
    rtk --help || true
    echo ""
    rtk --version || true
