...
#============================================================
# File: ripgrep.sh
# Description: 极速递归搜索文件内容的正则工具，遵循 gitignore 规则
...
download_exact() {
    ...
    TMP_DIR=$(mktemp -d /tmp/ripgrep.XXXXXX)
...
main() {
    ...
    if [[ -z "$DOWNLOAD_URL" ]]; then
        ...
        DOWNLOAD_URL="$(get_download_url BurntSushi/ripgrep "$OS_RE" "$ARCH_RE")"

        if [[ -z "$DOWNLOAD_URL" || "$DOWNLOAD_URL" == "null" ]]; then
            echo "Error: 未在 BurntSushi/ripgrep 的最新 release 中找到匹配 $OS-$ARCH 的安装包"
            exit 1
        fi
    else
    ...
    if ! check_is_command "ripgrep"; then
        echo "ripgrep has not been installed successfully."
        echo ""
        exit 1
    fi
