...
download_exact() {
    ...
    TMP_DIR=$(mktemp -d /tmp/fd.XXXXXX)
...
main() {
    ...
    if [[ -z "$DOWNLOAD_URL" ]]; then
        ...
        DOWNLOAD_URL="$(get_download_url sharkdp/fd "$OS_RE" "$ARCH_RE")"

        if [[ -z "$DOWNLOAD_URL" || "$DOWNLOAD_URL" == "null" ]]; then
            echo "Error: 未在 sharkdp/fd 的最新 release 中找到匹配 $OS-$ARCH 的安装包"
            exit 1
        fi
    else
    ...
    if ! check_is_command "fd"; then
        echo "fd has not been installed successfully."
        echo ""
        exit 1
    fi
