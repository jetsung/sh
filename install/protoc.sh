...
download_exact() {
    ...
    local file_bin="protoc"
    TMP_DIR=$(mktemp -d /tmp/protoc.XXXXXX)
...
main() {
    ...
    DOWNLOAD_URL="$(get_download_url protocolbuffers/protobuf)"

    download_exact    

    echo ""

    if ! check_is_command "protoc"; then
        echo "protoc has not been installed successfully."
        echo ""
        exit 1
    fi
