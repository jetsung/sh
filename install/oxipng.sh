...
#============================================================
# File: oxipng.sh
# Description: 多线程 Rust 编写的 PNG 压缩优化工具
# Source: https://github.com/oxipng/oxipng
# URL: https://fx4.cn/oxipng
...
download_exact() {
    local file_bin="oxipng"
    local download_file="package.tar.gz"
    TMP_DIR=$(mktemp -d /tmp/oxipng.XXXXXX)
...
main() {
    ...
    if [[ -z "$DOWNLOAD_URL" ]]; then
        ...
        DOWNLOAD_URL="$(get_download_url shssoichiro/oxipng "$OS_RE" "$ARCH_RE")"

        if [[ -z "$DOWNLOAD_URL" || "$DOWNLOAD_URL" == "null" ]]; then
            echo "Error: 未在 shssoichiro/oxipng 的最新 release 中找到匹配 $OS-$ARCH 的安装包"
            exit 1
        fi
    else
    ...
    if ! check_is_command "oxipng"; then
        echo "oxipng has not been installed successfully."
        echo ""
        exit 1
    fi

    echo "oxipng has been installed successfully!"
    echo ""
    oxipng --help || true
    echo ""
    oxipng --version || true
