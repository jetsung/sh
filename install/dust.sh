...
#============================================================
# File: dust.sh
# Description: 用 Rust 编写的更直观的磁盘占用分析（du）工具
# Source: https://github.com/bootandy/dust
# URL: https://fx4.cn/dust
...
download_exact() {
    ...
    TMP_DIR=$(mktemp -d /tmp/dust.XXXXXX)
...
main() {
    ...
    if [[ -z "$DOWNLOAD_URL" ]]; then
        ...
        DOWNLOAD_URL="$(get_download_url bootandy/dust "$OS_RE" "$ARCH_RE")"

        if [[ -z "$DOWNLOAD_URL" || "$DOWNLOAD_URL" == "null" ]]; then
            echo "Error: 未在 bootandy/dust 的最新 release 中找到匹配 $OS-$ARCH 的安装包"
            exit 1
        fi
    else
    ...
    if ! check_is_command "dust"; then
        echo "dust has not been installed successfully."
        echo ""
        exit 1
    fi

    echo "dust has been installed successfully!"
    echo ""
    dust --help || true
    echo ""
    dust --version || true
