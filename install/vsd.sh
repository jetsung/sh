...
#============================================================
# File: vsd.sh
# Description: HLS (m3u8) / DASH (mpd) 流媒体下载工具
# Source: https://github.com/clitic/vsd
...
main() {
    ...
    if [[ -z "$DOWNLOAD_URL" ]]; then
        ...
        DOWNLOAD_URL="$(get_download_url clitic/vsd "$OS_RE" "$ARCH_RE")"

        if [[ -z "$DOWNLOAD_URL" || "$DOWNLOAD_URL" == "null" ]]; then
            echo "Error: 未在 clitic/vsd 的最新 release 中找到匹配 $OS-$ARCH 的安装包"
            exit 1
        fi
    else
