...
download_exact() {
    ...
    local file_bin="aitr"
    TMP_DIR=$(mktemp -d /tmp/aitr.XXXXXX)
...
main() {
    ...
    DOWNLOAD_URL="$(get_download_url jetsung/ai-translator)"

    download_exact

    echo ""

    if ! check_is_command "aitr"; then
        echo "aitr has not been installed successfully."
        echo ""
        exit 1
    fi       

    echo ""
    echo "aitr has been installed successfully!"
    echo ""
    aitr --help
    echo ""
    aitr --version
