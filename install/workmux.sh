...
#============================================================
# File: workmux.sh
# Description: 基于 git worktree 与 tmux 窗口的零摩擦并行开发工具
# Source: https://github.com/raine/workmux
# URL: https://fx4.cn/workmux
...
download_exact() {
    local file_bin="workmux"
    local download_file="package.tar.gz"
    TMP_DIR=$(mktemp -d /tmp/workmux.XXXXXX)
...
main() {
    ...
    if [[ -z "$DOWNLOAD_URL" ]]; then
        ...
        DOWNLOAD_URL="$(get_download_url raine/workmux "$OS_RE" "$ARCH_RE")"

        if [[ -z "$DOWNLOAD_URL" || "$DOWNLOAD_URL" == "null" ]]; then
            echo "Error: 未在 raine/workmux 的最新 release 中找到匹配 $OS-$ARCH 的安装包"
            exit 1
        fi
    else
    ...
    if ! check_is_command "workmux"; then
        echo "workmux has not been installed successfully."
        echo ""
        exit 1
    fi

    echo "workmux has been installed successfully!"
    echo ""
    workmux --help || true
    echo ""
    workmux --version || true
    echo ""
}
