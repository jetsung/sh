...
#============================================================
# File: rimage.sh
# Description: 受 squoosh 启发的命令行图像压缩优化工具
# Source: https://github.com/vlad-salone/rimage
# URL: https://fx4.cn/rimage
...
download_exact() {
    local file_bin="rimage"
    local download_file="package.tar.gz"
    TMP_DIR=$(mktemp -d /tmp/rimage.XXXXXX)
...
main() {
    ...
    if [[ -z "$DOWNLOAD_URL" ]]; then
        ...
        DOWNLOAD_URL="$(get_download_url SalOne22/rimage "$OS_RE" "$ARCH_RE")"

        if [[ -z "$DOWNLOAD_URL" || "$DOWNLOAD_URL" == "null" ]]; then
            echo "Error: 未在 SalOne22/rimage 的最新 release 中找到匹配 $OS-$ARCH 的安装包"
            exit 1
        fi
    else
    ...
    if ! check_is_command "rimage"; then
        echo "rimage has not been installed successfully."
        echo ""
        exit 1
    fi

    echo "rimage has been installed successfully!"
    echo ""
    rimage --help || true
    echo ""
    rimage --version || true
