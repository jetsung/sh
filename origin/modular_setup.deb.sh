...
function die {
    ...
    echo -e "$prefix: "
    echo -e "$prefix: ${bold}You can contact Modular for further assistance.${reset}"
    echo -e "$prefix: "
...
function config_url {
    echo "https://dl.modular.com/public/installer/config.deb.txt?distro=${distro}&codename=${codename}&version=${version}&arch=${arch}" | sed 's/ /%20/g'
}
...
EOF
 ...
 >>>>: doesn't work after an hour, please contact Modular
 >>>>: for assistance.
 >>>>:
EOF
        }
...
function import_gpg_key {
    local text="Importing 'modular/installer' repository GPG keys ..."
    echo_running "$text"

    local gpg_keyring_path="/usr/share/keyrings/modular-installer-archive-keyring.gpg"
    
    curl -1sLf "https://dl.modular.com/public/installer/gpg.0E4925737A3895AD.key" | gpg --dearmor >> $gpg_keyring_path
    ...
    echo_okfail "Checking for apt signed-by key support ..." || {
        mv ${gpg_keyring_path} /etc/apt/trusted.gpg.d/modular-installer.gpg
    }
...
function setup_repository {
    local repo_path="/etc/apt/sources.list.d/modular-installer.list"
    check_fetch_config

    local text="Installing 'modular/installer' repository via apt ..."
    echo_running "$text"
...
echo "Executing the  setup script for the 'modular/installer' repository ..."
echo
